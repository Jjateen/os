...
void sjf_scheduler(const char* filename) {
    ...
    while (completed_processes < n) {
        ...
        // If CPU is idle, find the shortest job
        if (current_process == -1) {
            current_process = find_shortest_job(processes, n, current_time);
