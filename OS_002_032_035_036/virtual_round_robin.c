...
int read_processes_from_file(const char* filename, Process processes[]) {
    ...
    while (fgets(line, MAX_LINE_LENGTH, file) && count < MAX_PROCESSES) {
        ...
        processes[count].last_cpu_time = processes[count].arrival_time;
...
// Function to implement Virtual Round Robin scheduling
void virtual_round_robin_scheduler(const char* filename) {
    ...
    while (completed_processes < n) {
        ...
        if (current_process != -1) {
            int needs_io = processes[current_process].current_cpu_count > 0 && 
        ...
        if (current_process == -1 && queue_size > 0) {
            ...
            // Update waiting time: current time - last time it was in CPU
            if (processes[current_process].last_cpu_time >= 0) {
                processes[current_process].waiting_time += (current_time - processes[current_process].last_cpu_time);
            }
        }
    ...
    for (int i = 0; i < n; i++) {
        processes[i].waiting_time = processes[i].turnaround_time - processes[i].total_cpu_burst;
    }
    ...
    // Print timeline
    printf("\nExecution Timeline (- = idle, character = process ID):\n");
    
    // Print ruler for timeline
    printf("Time:  ");
    for (int i = 0; i < timeline_len; i += 5) {
        if (i < 100) {
            printf("%-5d", i);
        } else {
            printf("%-5d", i);
...
int main() {
    ...
    virtual_round_robin_scheduler(filename);
    return 0;
}